...
| 4 | **No automated tests for members** | Phase 3 shipped the entity, two value objects, five validators, the service and 12 endpoints with **zero** tests. The 51 unit tests cover books, paging and entities only. Every Phase 3 claim was verified by hand against a running server — reproducible, but not regression-proof. Highest-priority debt before Phase 4 builds lending on top of `Member.CanBorrow`. |
...
| 8 | `BookSearchRequest` date ranges unvalidated | `MemberSearchRequest` now refuses `joinedFrom > joinedTo`. The book search very likely has the same gap; unprobed. |
